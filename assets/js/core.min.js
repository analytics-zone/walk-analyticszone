...
(function () {
    ...
    // Suite Menu Toggle
    const suiteToggle = document.getElementById('suite-menu-toggle');
    const suiteDropdown = document.getElementById('suite-dropdown');

    if (suiteToggle && suiteDropdown) {
        suiteToggle.addEventListener('click', function (e) {
            e.stopPropagation();
            const isOpen = suiteDropdown.classList.toggle('hidden');
            suiteToggle.setAttribute('aria-expanded', !isOpen); // hidden removed = true
        });

        // Close when clicking outside
        document.addEventListener('click', function (event) {
            if (!suiteToggle.contains(event.target) && !suiteDropdown.contains(event.target)) {
                suiteDropdown.classList.add('hidden');
                suiteToggle.setAttribute('aria-expanded', 'false');
            }
        });
