...
const suiteToggle=document.getElementById('suite-menu-toggle');const suiteDropdown=document.getElementById('suite-dropdown');if(suiteToggle&&suiteDropdown){suiteToggle.addEventListener('click',function(e){e.stopPropagation();const isOpen=suiteDropdown.classList.toggle('hidden');suiteToggle.setAttribute('aria-expanded',!isOpen);});document.addEventListener('click',function(event){if(!suiteToggle.contains(event.target)&&!suiteDropdown.contains(event.target)){suiteDropdown.classList.add('hidden');suiteToggle.setAttribute('aria-expanded','false');}});document.addEventListener('keydown',function(event){if(event.key==='Escape'){suiteDropdown.classList.add('hidden');suiteToggle.setAttribute('aria-expanded','false');suiteToggle.focus();}});}
...
const themeToggle=document.getElementById('theme-toggle');const htmlElement=document.documentElement;if(themeToggle){themeToggle.addEventListener('click',function(){const isDark=htmlElement.classList.toggle('dark-mode');localStorage.setItem('theme',isDark?'dark':'light');updateThemeToggleButton(isDark);});}
function updateThemeToggleButton(isDark){if(!themeToggle)return;themeToggle.setAttribute('aria-label',isDark?'Switch to light mode':'Switch to dark mode');}
const savedTheme=localStorage.getItem('theme');if(savedTheme==='dark'){htmlElement.classList.add('dark-mode');updateThemeToggleButton(true);}else{updateThemeToggleButton(false);}
